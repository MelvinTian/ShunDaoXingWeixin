...
public class ThreeDes {
	...
	public static byte[] hexFromClient(String username){
		String key = "at97=!a";//关键字
		String f = DigestUtils.md5Hex(username+key);
		System.out.println("MD5后："+f);
	...
	public static byte[] hexToServer(String username){
		String key = "richFit!@#";//关键字
		String f = DigestUtils.md5Hex(username+key);
		System.out.println("MD5后："+f);
		byte[] bkeys = new String(f).getBytes();
		byte[] enk = new byte[24];
	...
	public static void main(String[] args) throws Base64DecodingException {
		...
		byte[] enk = hexFromClient("zeromike");//用户名
		...
		String rsPassword =  (new String(srcBytes));
		System.out.println("解密后的字符串:" +rsPassword);
		
		byte[] enk1 = hexToServer("zeromike");//用户名
		Security.addProvider(new com.sun.crypto.provider.SunJCE());
		System.out.println("加密前的字符串:" + rsPassword);
		byte[] encoded1 = encryptMode(enk1,rsPassword.getBytes());
		String pword1 = Base64.encode(encoded1); 
		System.out.println("加密后的字符串:" + pword1);
		
		byte[] reqPassword1 = Base64.decode(pword1);
		byte[] srcBytes1 = decryptMode(enk1,reqPassword1);
