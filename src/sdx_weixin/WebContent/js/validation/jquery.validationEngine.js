...
(function($) {
	...
	var methods = {
		...
		detach : function() {
			...
			// unbind fields
			/** edit by yangtao 2014.2.15 
			 * 
			 * form.off 应与 form.on 用法完全一样才能够删掉处理函数。
			 * 否则多次调用detach,attach方法如这样调用：
			 * .detach();
			 * .attach();
			 * .detach();
			 * .attach();
			...
			form.find("[" + options.validateAttribute + "*=validate]").not(
					"[type=checkbox]").off(options.validationEventTrigger,
					methods._onFieldEvent);
					*/
			form
			.off(
		...
		_validateField : function(field, options, skipAjaxValidation) {
			...
			if (!required && !(field.val()) && field.val().length < 1 
					/**
					 *  若字段有class="jqv.OverrideRequired"
					 *  即表示此列的required规则的优先级最低
					 *  
					 *  ajax校验可用此选项使ajax校验结果优先级高于required，
					 *  即当此列的值为空且没有required校验且ajax校验为false时，会出报错提示
					 *  
					 *  add by yangtao 2014.2.17
					 */
					&& !field.hasClass('jqv.OverrideRequired')
				){
				options.isError = false;
			}
		...
		_ajax : function(field, rules, i, options) {
			field.closest("form, .validationEngineContainer").trigger('jqv.ajaxfield.validating', [field]); // add by yangtao 2014.2.13
				...
				$
						.ajax({
							...
							success : function(json) {
								...
								// var errorField = $($("#" + errorFieldId)[0]);
								var errorField = $(document.getElementById(errorFieldId)).eq(0);
	...
	$(function() {
		$.validationEngine.defaults.promptPosition = methods.isRTL() ? 'bottomRight'
				: "bottomRight"
	});
})(jQuery);
